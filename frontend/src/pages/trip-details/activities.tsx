            ...
            {activities.map(activity => (
                <div key={activity.date} className="space-y-2.5">
                    <div className="flex gap-2 items-baseline">
                        <span className="text-xl text-zinc-300 font-semibold">Dia {parseInt(format(activity.date, 'd')) + 1}</span>
                        <span className="text-xs text-zinc-500">{titleCase(format(activity.date, 'EEEE', { locale: ptBR }))}</span>
                    </div>
